...
check-python:
	@command -v $(PYTHON) >/dev/null 2>&1 || (echo "ERROR: Python interpreter not found."; exit 1)
...
$(LOG_DIR)/syntax-check.stamp: $(DATA) $(ONTO) $(SHAPES) $(PREFIXES) $(FETCHED_DATA) tests/test_syntax.py | $(LOG_DIR) $(VENV)/.requirements-installed.stamp
	...
	@$(PYTEST) tests/test_syntax.py -q > /dev/null 2>&1 || (echo "Syntax check failed:" && $(PYTEST) tests/test_syntax.py -v && exit 1)
	@touch $@
